...
__global__ void assign_clustersModified(const float* __restrict__ data_x,
  ...
  float best_distance = squared_l2_distance(data_x[index], data_y[index], data_z[index], means_x[0],means_y[0],means_z[0]);
  int best_cluster = 0;
  for (int cluster = 1; cluster < numberOfClusters; cluster++) {
    float distance =squared_l2_distance(data_x[index], data_y[index], data_z[index], means_x[cluster],means_y[cluster],means_z[cluster]);
...
int main(int argc, char **argi){
  ...
  start = std::clock();
    ...
    assign_clustersModified<<<blocks, threads>>>(d_data.x,
      d_data.y,
      d_data.z,
      d_data.assignments,
      number_of_elements,
    ...
    size_t n = number_of_elements;
      ...
      do{
        ...
        reductionModified<BLOCKSIZE><<<blocksPerGrid,BLOCKSIZE>>>(d_means.x,
            ...
            n,
            numberOfClusters
          );checkCUDAError("Error during reduction");
      ...
      if (n > 1){
        ...
        reductionModified<BLOCKSIZE><<<1,BLOCKSIZE>>>(tmpx,
          ...
          tmpass,
            n,
            numberOfClusters
          );checkCUDAError("Error during last step reduction");
...
template<size_t blockSize>
__global__ void reductionModifiedS( float*  data_x,
  float*  data_y,
  float*  data_z,
  float*  data_assignments,
  float*  dataCopy_x,
  ...
  size_t data_size) {
    ...
    if (tid == 0){
      //const int cluster_index = blockIdx.x * numberOfClusters + cluster;
      dataCopy_x[blockIdx.x] = sdata[x];
      dataCopy_y[blockIdx.x] = sdata[y];
      dataCopy_z[blockIdx.x] = sdata[z];
      dataCopy_assignments[blockIdx.x] = sdata[ce];
    } 
  
}

__global__ void populateS( const float* __restrict__ data_x,
  const float* __restrict__ data_y,
  const float* __restrict__ data_z,
  float* data_assignments,
  const int  data_size,
...
__global__ void divideStepS( float*  dmx,
  float*  dmy,
  float*  dmz,
  float*  dma,
  float*  tmpx,
  ...
  int cluster) {
    ...
    dmx[cluster] = tmpx[0]/count;
    dmy[cluster] = tmpy[0]/count;
    dmz[cluster] = tmpz[0]/count;
  }
...
int main(int argc, char **argi){
  ...
  Data d_means(1 * number_of_elements, h_x, h_y, h_z, h_assignments);checkCUDAError("Error during d_means init");
  ...
  float* tmpx;
    cudaMalloc(&tmpx, sizeof(float) * blocksPerGrid * 1); checkCUDAError("Error allocating tmp [GPUReduction]");
    float* tmpy;
    cudaMalloc(&tmpy, sizeof(float) * blocksPerGrid * 1); checkCUDAError("Error allocating tmp [GPUReduction]");
    float* tmpz;
    cudaMalloc(&tmpz, sizeof(float) * blocksPerGrid * 1); checkCUDAError("Error allocating tmp [GPUReduction]");
    float* tmpass;
    cudaMalloc(&tmpass, sizeof(float) * blocksPerGrid * 1); checkCUDAError("Error allocating tmp [GPUReduction]");
  ...
  start = std::clock();
    ...
    for(int cluster = 0; cluster<numberOfClusters; cluster++){
      ...
      populateS<<<blocks, threads>>>(d_data.x,
        d_data.y,
        d_data.z,
        d_data.assignments,
        number_of_elements,
    ...
    do{
      ...
      reductionModifiedS<BLOCKSIZE><<<blocksPerGrid,BLOCKSIZE>>>(d_means.x,
          d_means.y,
          d_means.z,
          d_means.assignments,
          tmpx,
          ...
          //sostituire con funzione kernel che copia
          cudaMemcpy(d_means.x, tmpx, sizeof(float) * blocksPerGrid , cudaMemcpyDeviceToDevice);checkCUDAError("Error copying into tmpx");
          cudaMemcpy(d_means.y, tmpy, sizeof(float) * blocksPerGrid ,cudaMemcpyDeviceToDevice);checkCUDAError("Error copying into tmpy");
          cudaMemcpy(d_means.z, tmpz, sizeof(float) * blocksPerGrid ,cudaMemcpyDeviceToDevice);checkCUDAError("Error copying into tmpz");
          cudaMemcpy(d_means.assignments, tmpass, sizeof(float) * blocksPerGrid ,cudaMemcpyDeviceToDevice);checkCUDAError("Error copying into tmpass");
    ...
    if (n > 1){
      ...
      reductionModifiedS<BLOCKSIZE><<<1,BLOCKSIZE>>>(tmpx,
        tmpy,
        tmpz,
        tmpass,
        tmpx,
        ...
        tmpass,
          ...
          divideStepS<<<1,BLOCKSIZE>>>(d_means.x,
            d_means.y,
            d_means.z,
            d_means.assignments,
            tmpx,
            ...
            tmpass,
            n,
            cluster);checkCUDAError("Error during divideStep");
